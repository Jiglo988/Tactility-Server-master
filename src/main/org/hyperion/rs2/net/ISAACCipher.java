...
public class ISAACCipher {
	...
	public ISAACCipher(int[] seed) {
		byte[] newSeed = new byte[4*seed.length];
		for(int i = 0; i < seed.length; i++){
			newSeed[i] = ((byte) (seed[i] << 16));
			newSeed[i+1] = ((byte) (seed[i] << 8));
			newSeed[i+2] = ((byte) (seed[i] << 4));
			newSeed[i+3] = ((byte) (seed[i]));
		}
		secure = new SecureRandom(newSeed);
	}
	...
	private void initializeKeySet() {
		...
		int j2;
		int k2;
		int l = i1 = j1 = k1 = l1 = i2 = j2 = k2 = 0x9e3779c9;
		for(int i = 0; i < 4; i++) {
			l ^= i1 << 11;
			k1 += l;
			i1 += j1;
